    ...
    private T readResourceFromFile(File file) {
        try {
            ...
            if (fileData != null && fileData.getString("type").equals(resource.name())) {
                ...
                RosmontisLib.LOGGER.error("Failed to load static resource file {} from {}: ", file, this);
            }
        } catch (IOException e) {
            RosmontisLib.LOGGER.error("Failed to load static resource file {} from {}: ", file, this, e);
        }
        return null;
    }
}
