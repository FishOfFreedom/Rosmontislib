...
public class SynchedLevelEntityData {
   private static final Object2IntMap<Class<? extends LevelEntity>> ENTITY_ID_POOL = new Object2IntOpenHashMap<>();
   private final LevelEntity entity;
   private final Int2ObjectMap<DataItem<?>> itemsById = new Int2ObjectOpenHashMap<>();
   private final ReadWriteLock lock = new ReentrantReadWriteLock();
   private boolean isDirty;

   public SynchedLevelEntityData(LevelEntity pEntity) {
      this.entity = pEntity;
   }

   public static <T> EntityDataAccessor<T> defineId(Class<? extends LevelEntity> pClazz, EntityDataSerializer<T> pSerializer) {
      if (true) {
         try {
      ...
      if (ENTITY_ID_POOL.containsKey(pClazz)) {
         ...
         while(oclass1 != LevelEntity.class) {
   ...
   private <T> void createDataItem(EntityDataAccessor<T> pKey, T pValue) {
      SynchedLevelEntityData.DataItem<T> dataitem = new SynchedLevelEntityData.DataItem<>(pKey, pValue);
   ...
   private <T> SynchedLevelEntityData.DataItem<T> getItem(EntityDataAccessor<T> pKey) {
      this.lock.readLock().lock();

      SynchedLevelEntityData.DataItem<T> dataitem;
      try {
         dataitem = (SynchedLevelEntityData.DataItem<T>)this.itemsById.get(pKey.getId());
   ...
   public <T> void set(EntityDataAccessor<T> pKey, T pValue, boolean pForce) {
      SynchedLevelEntityData.DataItem<T> dataitem = this.getItem(pKey);
   ...
   public List<DataValue<?>> packDirty() {
      List<SynchedLevelEntityData.DataValue<?>> list = null;
      if (this.isDirty) {
         this.lock.readLock().lock();

         for(SynchedLevelEntityData.DataItem<?> dataitem : this.itemsById.values()) {
            if (dataitem.isDirty()) {
   ...
   @Nullable
   public List<SynchedLevelEntityData.DataValue<?>> getNonDefaultValues() {
      List<SynchedLevelEntityData.DataValue<?>> list = null;
      this.lock.readLock().lock();
   ...
   public void assignValues(List<SynchedLevelEntityData.DataValue<?>> pEntries) {
      this.lock.writeLock().lock();

      try {
         for(SynchedLevelEntityData.DataValue<?> datavalue : pEntries) {
            SynchedLevelEntityData.DataItem<?> dataitem = this.itemsById.get(datavalue.id);
            if (dataitem != null) {
   ...
   private <T> void assignValue(SynchedLevelEntityData.DataItem<T> pTarget, SynchedLevelEntityData.DataValue<?> pEntry) {
   ...
   public static class DataItem<T> {
      ...
      public SynchedLevelEntityData.DataValue<T> value() {
         return SynchedLevelEntityData.DataValue.create(this.accessor, this.value);
      }
   }

   public static record DataValue<T>(int id, EntityDataSerializer<T> serializer, T value) {
      public static <T> SynchedLevelEntityData.DataValue<T> create(EntityDataAccessor<T> pDataAccessor, T pValue) {
         EntityDataSerializer<T> entitydataserializer = pDataAccessor.getSerializer();
         return new SynchedLevelEntityData.DataValue<>(pDataAccessor.getId(), entitydataserializer, entitydataserializer.copy(pValue));
      }
      ...
      public static SynchedLevelEntityData.DataValue<?> read(FriendlyByteBuf pBuffer, int pId) {
      ...
      private static <T> SynchedLevelEntityData.DataValue<T> read(FriendlyByteBuf pBuffer, int pId, EntityDataSerializer<T> pSerializer) {
         return new SynchedLevelEntityData.DataValue<>(pId, pSerializer, pSerializer.read(pBuffer));
      }
   }
}